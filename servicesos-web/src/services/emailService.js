  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: buildPaymentConfirmationHTML(lead, payment),
      })
    });
    ...
    console.log(`[EMAIL] Payment confirmation sent to ${lead.email} | ID: ${data.id}`);
    return { success: true, id: data.id };
  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: buildFollowUpEmailHTML(lead, estimate),
      })
    });
    ...
    console.log(`[EMAIL] Follow-up sent to ${lead.email} | ID: ${data.id}`);
    return { success: true, id: data.id };
  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: buildAppointmentReminderHTML(lead, booking),
      })
    });
    ...
    console.log(`[EMAIL] Reminder sent to ${lead.email} | ID: ${data.id}`);
    return { success: true, id: data.id };
  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: buildServiceAgreementHTML(lead, estimate, contract),
        attachments: [
          {
    ...
    console.log(`[EMAIL] Service agreement sent to ${lead.email} | ID: ${data.id}`);
    return { success: true, id: data.id };
