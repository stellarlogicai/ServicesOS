 ...
 * Reusable across multiple SaaS products (Cleaning, FPS Coach, Anti-Cheat, etc.)
 */

import { doc, getDoc, setDoc, updateDoc, increment, collection, addDoc, query, where, orderBy, limit as limitQuery, getDocs } from 'firebase/firestore';
 ...
 */
export async function trackModelCost(tenantId, model, tokensIn, tokensOut, actualCost, operationType) {
  void operationType;
  try {
    await addDoc(collection(db, AI_CREDIT_HISTORY_COLLECTION), {
      tenantId,
 ...
 */
export async function getCreditHistory(tenantId, maxResults = 50) {
  try {
